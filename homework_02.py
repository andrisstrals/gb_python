...
l = len(mylist) & 0xfffffffe  # игнорируем последний нечётный елемент
...
params = (('название', str), ('цена', int), ('количество', int), ('eд', str))

goods = []

while True:
    task = input('Выберите\n\tД - добавить товар\n\tП - переидти к обработке ...')
    if task == 'П' or task == 'п':
        break
    elif task == 'Д' or task == 'д':
        item = {}
        for p in params:
            val = input(f'Введите "{p[0]}": ')
            item[p[0]] = p[1](val)
        goods.append((len(goods) + 1, item))
    else:
        print(f'Неудачный выбор, пробуйте ещё')
